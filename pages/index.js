...
{console.log(comments)}
  ...
  <Swiper 
    ...
    className="mySwiper">
    {comments.length > 0 && comments.map((oneComment)=>(
      <SwiperSlide key={oneComment.id}>
        <div className="swiper-icoon">
          <div className="swiper-icoon-grades">
            <p>{moment.parseZone(oneComment.datum).calendar()}</p>
            <h3 className="swiper-grades">{oneComment.rating}/10</h3>
          </div>
          <div className="swiper-icoon-grades">
            <Profiel className="icoon"  />
            <p>{oneComment.user.voornaam} {oneComment.user.naam}</p>
          </div>
          <div className="swiper-comment" dangerouslySetInnerHTML={{__html:oneComment.beschrijving}}></div>
        </div>
      </SwiperSlide>
    ))} 
    </Swiper>
    <div className="comment-add">
      ...
      <Link  href="/comment"><a title="ga naar comments"><Plus className="icoon-comment"/></a></Link>
    </div>
  </div>
  )
}
